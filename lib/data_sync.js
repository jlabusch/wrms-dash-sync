...
function push_org_data(cfg = config){
    // This isn't actually a promise...
    /* eslint-disable promise/catch-or-return */
    util.send_post({ url: cfg.get('api.host') + '/update_org_data' })
...
async function run(){
    if (first_run){
        ...
        try{
            ...
            wrms.on_error = pause;
            wrms.on_connect = unpause;
        }catch(ex){
            util.log(__filename, `ERROR with WRMS DB connection - ${ex}`);
        }
    ...
    return new Promise((resolve, reject) => {
        return get_contracts().then(
            contracts => {
                ...
                return util.promise_sequence(sql.delete_all, s => {
                    return sqlite_promise(dbs.syncing(), s);
                }).then(() => {
                    return util.promise_sequence(contracts, sync_contract, 0, util.ON_ERROR_CONTINUE).then(resolve, reject);
                });
            },
            reject
        );
...
function sync_contract(c){
    ...
    return new Promise((resolve, reject) => {
        return create_contract(dbs.syncing(), c).then(
            fetch_wrs_from_wrms(resolve, reject, c),
            su.soft_failure(resolve, `ERROR creating contract ${c.name}: `, __filename)
        );
    });
}
...
function fetch_wrs_from_wrms(resolve, reject, contract){
    return function(){
        ...
        return wrms.query(
            ...
            su.soft_failure(resolve, `ERROR getting WRs for systems ${JSON.stringify(contract.systems)}: `, __filename)
        ).catch(
            su.soft_failure(resolve, `ERROR getting WRs for systems ${JSON.stringify(contract.systems)}: `, __filename)
        );
    }
...
function fetch_quotes_from_wrms(resolve, reject, contract, wr_rows){
    ...
    return wrms.query(
        ...
        su.soft_failure(resolve, `ERROR getting quotes for systems ${JSON.stringify(contract.systems)}: `, __filename)
    ).catch(
        su.soft_failure(resolve, `ERROR getting quotes for systems ${JSON.stringify(contract.systems)}: `, __filename)
    );
}
...
function process_wrms_data(resolve, reject, contract, wr_rows){
    ...
    return async function(quotes){
        ...
        let has_quotes = {},
            approved_quote_time_for_this_wr = 0;
        ...
        for (let iw = 0; iw < wr_rows.length; ++iw){
            ...
            if (wr.request_id !== prev_wr.request_id){
                ...
                if (!wr.unchargeable){
                    ...
                    for (; iq < quotes.rows.length && quotes.rows[iq].request_id === wr.request_id; ++iq){
                        ...
                        // Note we've already converted days to hours at this point
                        ...
                        try{
                            ...
                            await sqlite_promise(dbs.syncing(), 'ROLLBACK').catch(err => {}); // eslint-disable-line no-unused-vars
                            continue;
                        }
                        ...
                        if (q.approved_by_id){
                            try{
                                approved_quote_time_for_this_wr += q.quote_amount;
                        ...
                        if (su.quote_may_affect_budget(q)){
            ...
            // Now add up the timesheets in the remaining rows for this WR.
            for (; iw < wr_rows.length && wr_rows[iw].request_id === wr.request_id; ++iw){
                ...
                let iw_hours = wr_rows[iw].timesheet_hours,
                    ...
                    iw_user = wr_rows[iw].timesheet_user;

                // { <wr_id>: {timesheet_total: n, people: {'bob smith': {timesheets: m}}}}
                let wr_contrib = contributors[wr.request_id] || {timesheet_total: 0, people: {}},
                    worker  = wr_contrib.people[iw_user] || {timesheets: 0};

                worker.timesheets += iw_hours;
                wr_contrib.people[iw_user] = worker;
                wr_contrib.timesheet_total += iw_hours;
                contributors[wr.request_id] = wr_contrib;
            ...
            if (['Finished', "For Sign Off", 'Production Ready', 'Testing/Signoff', 'Pending QA'].includes(wr.status) && !wr.unchargeable){
                let month_finished = util.dates.date_fmt(new Date(wr.status_on.replace(/T\d\d:/, 'T12:'))),
                    contrib = contributors[wr.request_id],
                    names = Object.keys(contrib.people);

                // for each staff member who contributed
                for (let ip = 0; ip < names.length; ++ip){
                    let name = names[ip],
                        // calculate their proportions of time spent
                        ratio = contrib.people[name].timesheets / contrib.timesheet_total,
                        // apply proportion to either sum of quotes or total time spent
                        portion = ratio * (approved_quote_time_for_this_wr || contrib.timesheet_total);

                    util.log_debug(__filename, sql.add_contribution + ' => ' + JSON.stringify([wr.request_id, month_finished, name, portion]));
                    if (portion > 0){
                        try{
                            await sqlite_promise(
                                dbs.syncing(),
                                sql.add_contribution,
                                wr.request_id,
                                month_finished,
                                name,
                                portion
                            ...
                            util.log(__filename, `ERROR adding contribution for ${name} on ${wr.request_id} for ${month_finished}: ${err}`);
                        }
                    }
                }
            }
...
function create_contract(db, c){
    return new Promise((resolve, reject) => {
        ...
        .then(resolve)  // eslint-disable-line indent
        .catch(reject); // eslint-disable-line indent
    });
}
    ...
    return new Promise((resolve, reject) => {
        dbs.syncing().all(
            ...
            (err, budgets) => {
                ...
                return sqlite_promise(dbs.syncing(), sql.add_budget, monthly_name, 0).then(
                    success => { // eslint-disable-line no-unused-vars
                        return sqlite_promise(dbs.syncing(), sql.add_contract_budget_link, contract.name, monthly_name).then(
                            success => { // eslint-disable-line no-unused-vars
                                return resolve({id: monthly_name, base_hours: 0, sla_quote_hours: 0, additional_hours: 0, base_hours_spent: 0});
                            },
                            reject
                        );
                    },
...
function pause(reason){
    if (reason){
        util.log(__filename, 'Pausing sync, reason ' + (reason || 'unknown'));
    }
    sync_active = false;
}

function unpause(){
    util.log(__filename, 'Unpausing sync');
    sync_active = true;
}
...
module.exports = {
    ...
    unpause: unpause,
    __test: {
        push_org_data: push_org_data,
        create_contract: create_contract,
        fetch_wrs_from_wrms: fetch_wrs_from_wrms
    }
}
