...
var __test_contracts = undefined;
...
function fetch_contracts(){
    return new Promise((resolve, reject) => {
        if (__test_contracts){
            process.nextTick(() => { resolve(__test_contracts); });
            return;
        }
...
function query_espo_contracts(context){
    return new Promise((resolve, reject) => {
        let token = Buffer.from(context.username + ':' + context.token).toString('base64'),
...
function query_espo_accounts(context){
    return new Promise((resolve, reject) => {
        let token = Buffer.from(auth_username + ':' + auth_token).toString('base64'),
            // TODO: increase offset until list.size is 0
            options = {
...
function merge_espo_data(context){
    ...
    context.contracts.list.forEach(c => {
        let is_active = c.status === 'Active'; // Possible statuses: Draft, Active, Closed/Completed

        if (c.type === 'Service Level Agreement'){
            ...
            if (!account_list[c.accountId]){
                account_list[c.accountId] = [];
            }
            account_list[c.accountId].push({
                name: c.name,
    ...
    context.accounts.list.forEach(acc => {
        ...
        // Create an org record whether there's a contract or not. data_sync's
        // add_new_contract_and_systems() will add more detail as necessary.
        // If there's no contract it'll never touch our DB, just be noted in org_data.
        ...
        if (account_list[acc.id]){
            account_list[acc.id].forEach(contract => {
                contract.org_id = acc.orgID;
                if (contract.active){
...
module.exports = {
    ...
    __test_override: (contracts) => { __test_contracts = contracts }
};
