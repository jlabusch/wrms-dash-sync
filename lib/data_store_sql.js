...
module.exports = {
    ...
    create_schema: [
        ...
        `CREATE TABLE earned_revenue (
            wr_id INTEGER,
            month_finished TEXT NOT NULL,
    ...
    dump: {
        contracts:  util.trim `
                    SELECT  c.id as contract_id,c.org_id,c.start_date,c.end_date,
                            cs.system_id,c.cash_value,c.cash_rate,c.cash_currency,c.active
                    FROM contracts c
    ...
    add_contract: 'INSERT OR REPLACE INTO contracts (id, org_id, org_name, start_date, end_date, cash_value, cash_rate, cash_currency, active) values (?, ?, ?, ?, ?, ?, ?, ?, ?)',
    ...
    add_contribution: 'INSERT INTO earned_revenue(wr_id, month_finished, worker, hours_earned) values (?, ?, ?, ?)',
    ...
    wrms: {
        ...
        wrs_for_system: (sys_arr) => {
            return `SELECT  r.request_id,
                            ...
                            (SELECT MAX(status_on) FROM request_status WHERE request_id=r.request_id) AS status_on,
                            ...
                            tsw.fullname as timesheet_user,
                            r.invoice_to
                    FROM request r
                    ...
                    LEFT JOIN usr tsw ON ts.work_by_id=tsw.user_no
                    WHERE r.system_id in (${sys_arr.join(',')})
                    ORDER BY r.request_id,ts.work_on`
                    .replace(/\s+/g, ' '); // eslint-disable-line indent
        },
